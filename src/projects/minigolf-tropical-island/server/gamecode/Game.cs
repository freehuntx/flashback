...
namespace BomberPenguGame
{
	...
	public class Player : BasePlayer
  {
    ...
    public string Skill
    {
      get { return string.Join("/", stats.Values); }
    }
		...
		private void OnXml(Player player, XmlDocument xml, string xmlString)
    {
			...
			if (rootName == "startGame")
      {
        ...
        //player.Send("xml", $"<startLevel nr=\"0\" pn=\"{player.Name}\" t=\"11\" />");
        //targetPlayer.Send("xml", $"<startLevel nr=\"0\" pn=\"{player.Name}\" t=\"11\" />");
        player.Send("xml", $"<startGame2 name=\"{targetPlayer.Name}\" />");
        targetPlayer.Send("xml", $"<startGame2 name=\"{targetPlayer.Name}\" />");
        //targetPlayer.Send("xml", $"<startGame name=\"{player.Name}\" />");
        //targetPlayer.Send("xml", $"<msgAll name=\"{player.Name}\" msg=\"asdasd\" />");
        //player.Send("xml", $"<msgAll name=\"{targetPlayer.Name}\" msg=\"asdasd2s\" />");
		...
		public override void GotMessage(Player player, Message message)
    {
      if (message.Type == "xml")
      {
        ...
        Console.WriteLine($"[XML] {player.Name}: {xmlString}");
